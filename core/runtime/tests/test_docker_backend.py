...
def test_docker_backend_real_container_lifecycle():
    ...
    spec = WorkloadSpec(workloadId="rt-dockertest", profile="test", env={"VEXA_X": "y"})
    try:
        rt.create(spec)
        assert rt.get("rt-dockertest").state is RuntimeState.running
        assert _exists(name)                                  # a REAL container is running

        rt.stop("rt-dockertest")
        assert rt.get("rt-dockertest").state is RuntimeState.stopped

        rt.destroy("rt-dockertest")
        assert rt.get("rt-dockertest").state is RuntimeState.destroyed
